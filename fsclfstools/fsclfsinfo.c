...
#endif
{
	const char *description = \
		"Use fsclfsinfo to determine information about a Common Log File System (CLFS) store.";

	fsclfstools_option_t options[ ] = {
		{ 'h', NULL, "shows this help" },
		{ 'v', NULL, "verbose output to stderr" },
		{ 'V', NULL, "print version" },
		{ 0, "source", "the source CLFS base log file" },
	};
	...
	int number_of_options      = (int) ( sizeof( options ) / sizeof( fsclfstools_option_t ) );
	int verbose                = 0;
	...
	if( fsclfstools_getopt_get_options_string(
	     options,
	     number_of_options,
	     options_string,
	     32 ) != 1 )
	                   ...
	                   options_string ) ) != (system_integer_t) -1 )
	{
		switch( option )
		{
			...
			default:
				...
				fsclfstools_getopt_usage_fprint(
				 stdout,
				 program,
				 description,
				 options,
