 ...
 */
int fsclfs_test_block_initialize(
     void )
{
	...
	block = (libfsclfs_block_t *) 0x12345678UL;
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libfsclfs_block_initialize with malloc failing
		 */
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libfsclfs_block_initialize with memset failing
		 */
...
/* Tests the libfsclfs_block_free function
 * Returns 1 if successful or 0 if not
 */
int fsclfs_test_block_free(
     void )
{
	libcerror_error_t *error = NULL;
	int result               = 0;
...
#endif
{
	...
	/* TODO: add tests for libfsclfs_block_get_sector_mapping_array_data */
