...
#include "../libfsclfs/libfsclfs_block_descriptor.h"

#if defined( __GNUC__ ) && !defined( LIBFSCLFS_DLL_IMPORT )

/* Tests the libfsclfs_block_descriptor_initialize function
 * Returns 1 if successful or 0 if not
 */
int fsclfs_test_block_descriptor_initialize(
     void )
{
	...
	/* Test regular cases
	 */
	result = libfsclfs_block_descriptor_initialize(
	          &block_descriptor,
	          &error );
	...
	result = libfsclfs_block_descriptor_free(
	          &block_descriptor,
	          &error );
	...
	block_descriptor = (libfsclfs_block_descriptor_t *) 0x12345678UL;
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libfsclfs_block_descriptor_initialize with malloc failing
		 */
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libfsclfs_block_descriptor_initialize with memset failing
		 */
...
/* Tests the libfsclfs_block_descriptor_free function
 * Returns 1 if successful or 0 if not
 */
int fsclfs_test_block_descriptor_free(
     void )
{
	libcerror_error_t *error = NULL;
	int result               = 0;
...
#endif
{
	...
	FSCLFS_TEST_RUN(
	 "libfsclfs_block_descriptor_initialize",
	 fsclfs_test_block_descriptor_initialize );

	FSCLFS_TEST_RUN(
	 "libfsclfs_block_descriptor_free",
	 fsclfs_test_block_descriptor_free );
